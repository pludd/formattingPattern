...
As the following examples illustrate, ~Formatted Description~ data type leverages the ~@formattingPattern~ XML attribute. All styled trade item information should be stored in ~@formattingPattern~, while its corresponding unstyled (plain text) information should be stored in its parent XML element.
